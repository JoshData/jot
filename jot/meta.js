			...
			for (var j = new_ops.length-1; j >= 0; j--) {
				...
				if (c) {
					...
					if (j > 0 && op.rebase_inv) {
						...
						// be rebased on new_ops[j]'s inverse. Operations
						// don't all support getting their inverse, so
						// we have a special function for rebase-on-inverse.
						var r1 = op.rebase_inv(new_ops[j]);
						var r2 = new_ops[j].rebase_inv(op);
						if (r1 != null && r2 != null) {
